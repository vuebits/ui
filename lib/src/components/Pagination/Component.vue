  ...
  >
    ...
    >
      <slot name="jumper">
        ...
        <span :class="$bem({ e: 'jumper-item' })"> {{ dict.total }} {{ lastPage }} </span>
      </slot>
    </div>
  </div>
</template>
...
export default defineComponent({
  ...
  computed: {
    ...
    pageButtons(): PageButton[] {
      ...
      const pageButtonsAmount = Math.min(maxPageButtons, this.lastPage)
      const pageButtons = []
      ...
      for (let i = 0; i < pageButtonsAmount; i++) {
        ...
        if (i === 0) {
        ...
        } else if (this.currentPage < maxPageButtons - 2) {
        ...
        } else if (this.currentPage > this.lastPage - maxPageButtons + 3) {
          ...
          const firstMiddlePageButtonValue =
            this.currentPage - Math.round((middlePageButtonsAmount - 1) / 2)
          if (i === 1) {
            // second button
            pageButton = {
