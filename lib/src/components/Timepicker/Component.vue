<template>
  ...
  >
    ...
    />
    <span :class="bem({e: 'time-separator'})">
      :
    </span>
    <VInput
...
const bem = createBem('v-timepicker');
...
const chunkTimeItemString = (v: number) => isTempTimeEmpty.value ? '' : `00${v ?? 0}`.substr(-2);
...
const timeObject = computed({
  ...
  set (obj: TimeObject | null): void {
    ...
    ].map(v => chunkTimeItemString(v)).join(':');
...
const isMinHourSelected = computed(() => tempTimeObject.hour === disableBeforeObject.value?.hour);
const isMaxHourSelected = computed(() => tempTimeObject.hour === disableAfterObject.value?.hour);
const minMinute = computed(() => isMinHourSelected.value ? disableBeforeObject.value?.minute : null ?? 0);
const maxMinute = computed(() => isMaxHourSelected.value ? disableAfterObject.value?.minute : null ?? 59);
const isMinMinuteSelected = computed(() => isMinHourSelected.value && tempTimeObject?.minute === disableBeforeObject.value?.minute);
const isMaxMinuteSelected = computed(() => isMaxHourSelected.value && tempTimeObject?.minute === disableAfterObject.value?.minute);
const minSecond = computed(() => isMinMinuteSelected.value ? disableBeforeObject.value?.second : null ?? 0);
const maxSecond = computed(() => isMaxMinuteSelected.value ? disableAfterObject.value?.second : null ?? 59);
  ...
  if (disableBeforeObject.value) {
    if (hour < minHour.value) {
      ...
      if (minMinute.value !== undefined && minute < minMinute.value) {
        setTempTime(disableBeforeObject.value);
      } else if (isMinMinuteSelected.value && minSecond.value !== undefined && second < minSecond.value) {
  ...
  if (disableAfterObject.value) {
    if (hour > maxHour.value) {
      ...
      if (maxMinute.value !== undefined && minute > maxMinute.value) {
        setTempTime(disableAfterObject.value);
      } else if (isMaxMinuteSelected.value && maxSecond.value !== undefined && second > maxSecond.value) {
